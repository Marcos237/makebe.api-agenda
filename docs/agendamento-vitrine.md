...
Além da implementação, criar os testes unitários na camada de testes seguindo o padrão já utilizado no projeto.

Cobrir no mínimo os seguintes cenários:

### Geração de Períodos

* Deve gerar períodos corretamente quando não houver bloqueios.
* Deve gerar períodos corretamente respeitando o valor de `Servico.Periodo`.
* Deve gerar períodos de 24 horas quando não existirem restrições.
* Deve gerar períodos em intervalos de 15, 30, 45 e 60 minutos.

### Período Inativo

* Deve remover horários dentro do período inativo.
* Deve tratar período inativo que atravessa a meia-noite.
* Deve manter apenas horários válidos fora do período inativo.
* Não deve remover horários quando período inativo não estiver preenchido.

### Agenda Bloqueada

* Deve remover horários dentro da agenda bloqueada.
* Deve tratar múltiplos períodos bloqueados.
* Deve permitir horários fora dos períodos bloqueados.
* Não deve remover horários quando não houver agenda bloqueada.

### Agendamentos Existentes

* Deve marcar `IsAgendado = true` quando existir agendamento para o período.
* Deve manter `IsAgendado = false` quando não existir agendamento.
* Deve tratar múltiplos agendamentos no mesmo dia.
* Deve ignorar registros sem `DataInicioAgendamento`.

### Cenários Limite

* Deve retornar lista vazia quando não houver períodos disponíveis.
* Deve tratar corretamente períodos sobrepostos entre bloqueios e períodos inativos.
* Deve tratar corretamente início e fim exatamente nos limites dos horários.
* Deve garantir que não existam períodos duplicados.
* Deve garantir que os períodos retornados estejam ordenados por data/hora de início.

Utilizar os frameworks e padrões já existentes no projeto (xUnit, FluentAssertions, Mocks, Builders, Fixtures, etc.), sem criar uma abordagem diferente da adotada atualmente.
